  ...
  const activeFilterChips = [
    ...
    ...(activeMinPrice || activeMaxPrice
      ? [
          {
            key: "price",
            label: `Precio ${activeMinPrice || "0"} - ${activeMaxPrice || "max"}`,
      ...
      <section className={`overflow-hidden relative pb-20 pt-5 lg:pt-20 xl:pt-28 bg-[#f3f4f6] ${previewMode && selectionMode ? "lumefy-preview--selecting" : ""}`}>
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
            {/* <!-- Sidebar Start --> */}
            <div
              className={`sidebar-content ${catalogPreviewClass(filtersSection.id)} fixed xl:z-1 z-9999 left-0 top-0 xl:visible xl:pointer-events-auto xl:translate-x-0 xl:static max-w-[310px] xl:max-w-[270px] w-full ease-out duration-200 ${
                productSidebar
                  ? "visible pointer-events-auto translate-x-0 bg-white p-5 h-screen overflow-y-auto"
                ...
                <div className="mb-4 rounded-lg border border-gray-3 bg-white px-3 py-3 shadow-1 sm:px-4">
                  ...
                  {activeFilterChips.map((chip) => (
                    <button
                      ...
                      className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-gray-3 bg-gray-1 px-2.5 py-1 text-xs font-medium text-dark transition hover:border-dark hover:bg-white sm:px-3 sm:py-1.5 sm:text-sm"
                    >
                      <span className="truncate">{chip.label}</span>
                      <span className="text-sm leading-none text-dark-4 sm:text-base">x</span>
                    </button>
                  ))}
                  </div>
                </div>
